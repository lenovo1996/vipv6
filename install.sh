...
BIN_EXEC="${BIN_DIR}vipv6"
WORKDIR="/etc/lev/"
WORKDATA="${WORKDIR}/data.txt"
LOGFILE="/var/log/vipv6.log"

cat <<"EOF"
==========================================================================
IPv6 All In One v1.0
==========================================================================
EOF
...
upload_proxy() {
  ...
  zip --password $PASS vipv6_proxy.zip proxy.txt >/dev/null
  URL=$(curl -s -F "file=@vipv6_proxy.zip" https://file.io | jq '.link')
...
while [[ $selection -ne 10 ]]; do
  ...
  currentIPv6List=$(ip addr show dev ${network_card} | sed -e's/^.*inet6 \([^ ]*\).*$//;t;d' | grep -v fe80)
  firstIPv6=$(ip addr show dev ${network_card} | sed -e's/^.*inet6 \([^ ]*\).*$//;t;d' | grep -v fe80 | head -n 1)
  if [[ "$firstIPv6" ]]; then
    ipv6=$(echo $firstIPv6)
...
et.ipv6.conf.default.disable_ipv6 = 0" /etc/sysctl.conf
    ...
    echo "$currentIPv6List" | while IFS= read -r line; do
    ...
    if [[ "$GWCheck" = "NOGW" ]]; then
      ...
      if [[ "$OS" = "CentOS Linux" ]]; then
        if [[ -z $(yum list installed | grep 3proxy) ]]; then
          ...
          mkdir -p ./3proxy
          cd ./3proxy
          wget -q https://github.com/lenovo1996/vipv6/raw/master/3proxy-centos.rpm
          temp=$(rpm -i 3proxy-centos.rpm)
          ...
          sed -i "/Description=/c\Description=3 Proxy" /etc/sysctl.conf
